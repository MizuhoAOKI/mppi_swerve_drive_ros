...
)
{
    ...
    {
        // initialize state
        State x = target_system::convertXYYawToStateSpace3D(observed_state); // in mppi_4d, StateSpace3D is equal to XYYaw
    ...
    common_type::VxVyOmega optimal_vxvyw_cmd = target_system::convertControlSpace4DToVxVyOmega(u_opt_seq[0], param_);
...
{
    return target_system::convertControlSpace4DToControlSpace8D(u_opt_latest_, param_);
}

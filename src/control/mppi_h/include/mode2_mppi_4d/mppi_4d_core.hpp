...
using State = target_system_mppi_4d::StateSpace3D;
using StateSeq = std::vector<State>;
using StateSeqSamples = std::vector<StateSeq>;
using Control = target_system_mppi_4d::ControlSpace4D;
...
{
    public:
        MPPI4DCore(param::CommonParam& param_common, param::MPPI4DParam& param);
        ~MPPI4DCore();
        ...
        param::MPPI4DParam getParam() { return param_; }
